...
#' @title Load the table of admin names
#'
#' @param filepath \code{(character)} path to admin table file
#'
#' @return \code{(tibble)}

get_admins <- function(filepath = fs::path("data", "admins", ext = "feather")) {
  arrow::read_feather(filepath)
}

#' @title Checks if a first name / last name pair is an admin
#' @description Sets global `user_profile` state `admin = TRUE`
#' @param first_name \code{(character)}
#' @param last_name \code{(character)}
#'
#' @return \code{(logical)}

is_admin <- function(first_name, last_name) {
  out <- nrow(dplyr::filter(get_admins(), first_name == first_name & last_name == last_name)) > 0
  if (out)
    .GlobalEnv$user_profile$admin = TRUE
  out
}
